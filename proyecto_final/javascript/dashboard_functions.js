document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchStockGainAPI(endpoint, options = {}) {
        ...
        // Solo añadimos el token si no es una petición a la API de precios
    ...
    const updatePriceForSelectedAsset = async () => {
        ...
        try {
            ...
            const result = await fetchStockGainAPI(`/price/${selectedTicker}`);
            ...
            console.error(`Error obteniendo precio para ${selectedTicker}:`, error);
            currentAssetPriceEl.textContent = 'No disponible';
        }
    };
    ...
    const updateAllAssetPrices = async () => {
        ...
        for (const asset of userPortfolio) {
            try {
                // Ahora llamamos a nuestra propia API para cada activo
                const result = await fetchStockGainAPI(`/price/${asset.ticker}`);
                if (result && result['Global Quote'] && result['Global Quote']['05. price']) {
                ...
                console.error(`Fallo al obtener precio para ${asset.ticker}`, error);
                asset.precioActual = asset.precioActual || 0;
            }
        }
